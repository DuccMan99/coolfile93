...
<head>
    ...
    <link rel="stylesheet" href="/sys/hotfix.css?v=2.4.2">
</head>
<script>
...
<body>
    <div id="trollbox" class="skin_base ui_layout _ui_unselectable">
        ...
        <script src="//www.windows93.net:8081/socket.io/socket.io.js"></script>
        ...
        <script>
                    ...
                    if (cmd) {
                        ...
                        if (cmd.cmd === 'voice' && voices.length > 0) {
                            ...
                            dada = {
                                ...
                                style: "opacity: 0.7;",
                                home: 'local',
                                msg: "Voice set to: " + voices[voice].name
                            };
                            printMsg(dada);
                            return;
                        }
            ...
            function printMsg(data) {
                ...
                if (data.home == undefined || data.home == null || !data.home) {
                    return;
                };
                for (var i = 0; i < blocked.length; i++) {
                    if (data.home == blocked[i]) {
                ...
                if ((data.msg) && (data.msg.startsWith('data:image/'))) {
                    if (imgShow) {
                        if (data.msg.indexOf("&#62") != -1) {
                            return
                        };
                        if (data.msg.indexOf("&#39") != -1) {
                            return
                        };
                ...
                if (words[0] == "/say") {
                    ...
                    say.rate = settings[1];
                    if (voices.length > 0) {
                        say.voice = voices[parseInt(settings[2]) % voices.length]
                    };
            ...
            socket.on('user change nick', function(data) {
                if (data[0].nick == data[1].nick) {
                    return
                };
                for (var i = 0; i < blocked.length; i++) {
                    if (data[1].home == blocked[i]) {
                        return
                    }
                };
                if (data[1].nick) printMsg({
                    date: Date.now(),
                    color: '#af519b',
                    nick: '~',
                    home: data[1].home,
                    msg: printNick(data[0]) + ' <em>is now known as</em> ' + printNick(data[1])
                });
            });
            ...
            function send(e) {
                ...
                if (some.match(/fortnite/i)) {
                    if (parent.$explorer) {
                        parent.le._dom.background.style.background = "#000";
                        parent.$alert({
                            msg: "This copy of Windows 93 is not Jenuine.\nActivate Windows 93 today for the low low price of €99999999!!!",
                            title: "Windows93",
                            img: "/c/sys/skins/w93/apps/keygen.png",
                            btnOk: "I'm all out of moola"
                        });
                        parent.$fs.utils.normalizeIcon = function() {
                            return "/c/sys/skins/w93/apps/keygen.png"
                ...
                if (messageStyle == "normal") {
                    sendMsg(trollbox_input.value)
                };
                if (messageStyle == "l337") {
                    sendMsg(tol33t(trollbox_input.value))
                };
                if (messageStyle == "upDown") {
                    sendMsg(flipText(trollbox_input.value))
                };
        ...
        <script>
            ...
            function lorem(numb) {
                ...
                for (var i = 0; i < numb; i++) {
                    ...
                    str = str + lW;
                    if ((parseInt(Math.random() * 5) == 1) && (i != numb - 1)) {
                        str = str + ","
                    };
                    str = str + " ";
                };
